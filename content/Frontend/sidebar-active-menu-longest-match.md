...
활성 메뉴 판정을 `route.path.startsWith(item.path)`로 하면, 메뉴 경로끼리 접두어 관계일 때(`/admin/devices` vs `/admin/devices/stored`) 하위 페이지에서 **두 메뉴가 동시에 활성**된다.
...
- `path + '/'` 경계 매칭으로 `/admin/devices2` 같은 유사 접두어 오매칭도 함께 제거
