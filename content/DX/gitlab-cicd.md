...
여러 컴포넌트를 한 레포에서 릴리스할 때의 루트 `.gitlab-ci.yml` 오케스트레이션 패턴 → [[gitlab-ci-monorepo-root-pattern]]

---

## 함정 — 런타임 의존성은 베이스 이미지에 베이킹

도구가 런타임에 다른 도구를 호출할 때 생기는 실패 패턴과 해결법 → [[gitlab-ci-runtime-dependency-baking]]
...
- [[gitlab-ci-monorepo-root-pattern]] — 모노레포 루트 CI 오케스트레이션 패턴
- [[gitlab-ci-runtime-dependency-baking]] — 런타임 의존성 베이스 이미지 베이킹
- [[dx-overview]]
