...
> **L1·L2·L3**은 OSI 계층 번호다 — L1은 물리(케이블·전기신호), L2는 데이터링크(MAC 주소·스위치·VLAN), L3은 네트워크(IP·라우팅). 나머지 약어는 [[network-overview]] 용어 표 참고.
...
`RX`는 Receive(수신), `TX`는 Transmit(송신)이다.
...
> [!WARNING]
> - 스위치 포트를 옮긴 직후 잠깐 통신이 안 되는 것은 STP(Spanning Tree Protocol, 루프 방지 프로토콜)의 수렴과 MAC 주소 학습 지연 때문으로 정상 (수십 초 대기)
