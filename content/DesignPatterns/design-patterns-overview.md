...
- [[uniform-deadline-kills-long-legitimate-work]] — 일괄 마감은 종류별 정상 소요가 다른 작업을 죽인다: 요청별 마감을 상태 전이와 같은 문장에서 행에 적는다(실증)
- [[argv-assembly-single-gate]] — 플래그 규약을 호출 지점마다 지키게 하면 샌다: `sudo` 뒤 인자 위치는 소스 검사를 통과한다, 조립을 한 모듈에 가둔다(실증)
